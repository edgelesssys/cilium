...
func (h *HeaderfileWriter) WriteNodeConfig(w io.Writer, cfg *datapath.LocalNodeConfiguration) error {
	...
	if option.Config.StrictModeCIDR != nil {
		...
		cDefinesMap["STRICT_IPV4_NET"] = fmt.Sprintf("%#x", byteorder.NetIPv4ToHost32(option.Config.StrictModeCIDR.IP))
		ones, _ := option.Config.StrictModeCIDR.IPNet.Mask.Size()
		cDefinesMap["STRICT_IPV4_NET_SIZE"] = fmt.Sprintf("%d", ones)
	}
